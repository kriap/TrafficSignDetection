...
Computes region loss from network output and target annotation. (https://eavise.gitlab.io/lightnet/index.html)
...
class YoloLoss(nn.modules.loss._Loss):
    ...
    def forward(self, output, target):
        ...
        anchor_h = anchor_h.type(torch.DoubleTensor)
        ...
        coord_mask = coord_mask.type(torch.DoubleTensor)
        conf_mask = conf_mask.type(torch.DoubleTensor)
        tcoord = tcoord.type(torch.DoubleTensor)
        tconf = tconf.type(torch.DoubleTensor)
